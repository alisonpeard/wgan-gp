...
def wrap_class(cls, device, **kwargs):
  ...
  class NumpyOpWrapper(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, values, device=device):
      obj = cls(values.cpu().numpy(), **kwargs)
      ctx.numpy_obj = obj
      return torch.from_numpy(obj.compute()).to(device)

    @staticmethod
    def backward(ctx, grad_output, device=device):
      return torch.from_numpy(ctx.numpy_obj.vjp(grad_output.cpu().numpy())).to(device)
  
  return NumpyOpWrapper


def map_tensor(map_fn, tensor, device):
...
def soft_rank(values, direction="ASCENDING", regularization_strength=1.0,
  ...
  wrapped_fn = wrap_class(numpy_ops.SoftRank,
                          ...
                          device=device)
  return map_tensor(wrapped_fn.apply, values, device)
